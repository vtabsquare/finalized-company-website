...
const html=`<div style="font-family:Arial,sans-serif;max-width:720px"><h2>VTAB Square Daily Visitor Intelligence</h2><p><strong>Likely human page requests:</strong> ${report.likely_human_page_requests}<br><strong>Bot requests:</strong> ${report.bot_requests}<br><strong>Scanner requests:</strong> ${report.scanner_requests}<br><strong>Service-interest requests:</strong> ${report.service_interest_requests}<br><strong>Contact page requests:</strong> ${report.contact_page_requests}<br><strong>Tracked CTA actions:</strong> ${report.conversion_actions}<br><strong>Enquiries received:</strong> ${report.enquiries_received || 0}<br><strong>Conversion rate:</strong> ${report.conversion_rate_per_100_page_requests}%</p><h3>Enquiry interests</h3><ul>${list(report.enquiry_interests)}</ul><h3>Conversions</h3><ul>${list(report.conversions)}</ul><h3>Top pages</h3><ul>${list(report.top_pages)}</ul><h3>External referrers</h3><ul>${list(report.external_referrers)}</ul><p><strong>Suggested next action:</strong> ${report.next_action}</p><p style="color:#666;font-size:12px">Aggregate operational analytics only. No raw IP addresses are included.</p></div>`;
const res=await fetch('https://api.brevo.com/v3/smtp/email',{method:'POST',headers:{'api-key':apiKey,'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({sender:{name:senderName,email:senderEmail},to:recipients.map(email=>({email})),subject:`VTAB Square Daily Visitor Intelligence - ${process.env.REPORT_PREVIOUS_DAY === '1' ? new Date(Date.now()-86400000).toLocaleDateString('en-IN',{timeZone:'Asia/Kolkata'}) : new Date().toLocaleDateString('en-IN',{timeZone:'Asia/Kolkata'})}`,htmlContent:html})});
