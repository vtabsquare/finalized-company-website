...
const IOT_SERVER_ROOM_INNOVATION: Innovation = {
  id: 'iot-ai-server-room-monitoring',
  title: 'IoT-Based AI Server Room Monitoring',
  tagline: 'Always-on intelligence for critical infrastructure',
  description: 'An AI-assisted IoT monitoring system that combines environmental sensors, equipment telemetry, and anomaly detection to identify server-room risks before they become outages.',
  icon: 'Cpu',
  highlights: [
    'Real-time temperature, humidity, airflow, smoke, and power monitoring',
    'AI anomaly detection with predictive equipment-risk alerts',
    'Instant escalation workflows for threshold breaches and device failures',
    'Unified dashboards for trends, incidents, and infrastructure health',
  ],
...
export const FutureInnovations: React.FC<FutureInnovationsProps> = ({
  ...
  const [innovations, setInnovations] = useState<Innovation[]>([IOT_SERVER_ROOM_INNOVATION]);
  ...
  useEffect(() => {
    ...
    const fetchInnovations = async () => {
      ...
      if (error) {
        ...
        const remoteInnovations = data as Innovation[];
        setInnovations([
          IOT_SERVER_ROOM_INNOVATION,
          ...remoteInnovations.filter(item => item.id !== IOT_SERVER_ROOM_INNOVATION.id),
        ]);
      }
      setLoading(false);
    };
