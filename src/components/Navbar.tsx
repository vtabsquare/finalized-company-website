...
import { trackClick } from '../lib/analyticsService';
...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const handleNavClick = (tabId: NavTab) => {
    ...
    trackClick('Nav Bar Link', { target_tab: tabId });
  ...
  const navActiveClass = useHeroNav && !isLightMode
    ? 'text-white'
    : isLightMode ? 'text-slate-950' : 'text-white';

  const navInactiveClass = useHeroNav && !isLightMode
    ? 'text-slate-300 hover:text-white'
    : isLightMode ? 'text-slate-600 hover:text-slate-950' : 'text-slate-400 hover:text-white';

  const navUnderlineClass = !isLightMode ? 'bg-white' : 'bg-slate-950';

  const logoTitleClass = useHeroNav && !isLightMode
    ? 'text-white group-hover:text-cyan-100'
    : isLightMode ? 'text-slate-950 group-hover:text-blue-700' : 'text-white group-hover:text-amber-300';

  const logoAccentClass = useHeroNav && !isLightMode
    ? 'text-cyan-400 font-extrabold'
    : isLightMode ? 'text-blue-700 font-extrabold' : 'text-amber-400 font-extrabold';

  const themeToggleClass = useHeroNav && !isLightMode
    ? 'text-slate-300 hover:text-white'
    : isLightMode ? 'text-slate-500 hover:text-slate-950' : 'text-slate-500 hover:text-white';

  const headerCtaClass = useHeroNav && !isLightMode
    ? 'bg-white/95 text-slate-950 hover:bg-white border border-white/20'
    : isLightMode ? 'bg-slate-950 text-white hover:bg-slate-800 border-transparent shadow-md' : 'bg-white text-slate-950 hover:bg-slate-200';

  const mobileMenuBtnClass = useHeroNav && !isLightMode
    ? 'text-white hover:bg-white/10'
    : isLightMode ? 'text-slate-700 hover:bg-slate-100' : 'text-slate-300 hover:bg-white/[0.06]';

  const mobileThemeClass = useHeroNav && !isLightMode
